...
if erisPLL_inventoryBar then
	if erisPLL_inventoryBar.versionNumber >= versionNumber then
		return;
	end;
end
...
erisPLL_inventoryBar.registerItem = function(_itemFullType, _modData, _label)
	erisPLL_inventoryBar.drawForItems[_itemFullType] = {
		modData = _modData,
		label = _label,
	};
end
...
erisPLL_inventoryBar.doOverride = function()
	...
	ISInventoryPane.drawItemDetails = function(self, _item, _y, _xoff, _yoff, _red)
		if _item then
			local drawItem = erisPLL_inventoryBar.drawForItems[_item:getFullType()];
			if drawItem then
...
erisPLL_inventoryBar.init = function()
	erisPLL_inventoryBar.doOverride();
end

----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------

Events.OnGameStart.Add(erisPLL_inventoryBar.init);
